...
class Router():
    ...
    def print_table(self, it, f):
        if it == -1:
            epoch = 'Final state'
        else:
            epoch = f'Simulation step {it}'
        f.write(f'{epoch} of router {self.ip} table:\n')
        f.write('[Source IP]          [Destination IP]     [Next Hop]      [Metric]\n')
        for ip in ips:
            if ip != self.ip and not np.isinf(self.table.dists[ip]):
                f.write(f'{self.ip:20} {ip:20} {self.table.next[ip]:20} {self.table.dists[ip]:3}\n')
        f.write('\n')

def thread_func(router):
    f = open(f'logs/{router.ip}.txt', 'w')

    router.init_with_inf()
    time.sleep(5)
    n = len(ips)
    for it in range(n):
        router.update_neighbours_tables()
        time.sleep(15)
        router.update_table()
        time.sleep(15)
        router.print_table(it, f)
    router.print_table(-1, f)
    f.close()
...
def main():
    ...
    with open(args.filename, 'r') as file:
        ...
        threads = []
        for ip in ips:
            threads.append(threading.Thread(group=None, target=thread_func, name=ip, args=([routers[ip]])))
        for thr in threads:
            thr.start()
